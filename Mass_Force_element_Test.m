...
%opt=odeset('RelTol',1,'AbsTol',1,'MaxStep',1);
opt=odeset('RelTol',1*10^-2,'AbsTol',1*10^-3,'MaxStep',1);

%opt=odeset('RelTol',1*10^-4,'AbsTol',1*10^-5,'MaxStep',1);
n_AE0 = 5;
m_LJ = 5;
number_AE0 =n_AE0-1;%给下面那根长杆AE0部分加入几个关节
number_LJ = m_LJ-1;%给上面那根长杆FL部分加入几个关节
...
F1 = 0;F2 = 182663;F3 = 150180; %输入为两个驱动力；单位N；静止状态
...
tic;
[t_set,x]=ode23tb(@(t,x)Multi_Body_Dynamic_con(t,x,u,F,r,L,number_AE0,number_LJ,m),[0 20],x0,opt);%ode方程求积分
